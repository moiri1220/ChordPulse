"""BTC (Bi-directional Transformer for Musical Chord Recognition) モデル定義。

論文: "A Bi-Directional Transformer for Musical Chord Recognition" (ISMIR 2019)
参照実装: https://github.com/jayg996/BTC-ISMIR19
"""
...
class OutputLayer(nn.Module):
    ...
    def loss(self, hidden, labels):
        raise NotImplementedError(f'Must implement {self.__class__.__name__}.loss')
...
class bi_directional_self_attention_layers(nn.Module):
    def __init__(self, embedding_size, hidden_size, num_layers, num_heads, total_key_depth, total_value_depth,
        ...
        self.self_attn_layers = nn.Sequential(
            *[bi_directional_self_attention(*params) for _layer_idx in range(num_layers)]
        )
...
class BTC_model(nn.Module):
    def __init__(self, config: dict | None = None):
        ...
        self.timestep = cfg.get("timestep", 108)
        self.probs_out = cfg.get("probs_out", False)

        params = (
            cfg.get("feature_size", 144),
            cfg.get("hidden_size", 128),
            cfg.get("num_layers", 8),
            cfg.get("num_heads", 4),
            cfg.get("total_key_depth", 128),
            cfg.get("total_value_depth", 128),
            cfg.get("filter_size", 128),
            cfg.get("timestep", 108),
            cfg.get("input_dropout", 0.2),
            cfg.get("layer_dropout", 0.2),
            cfg.get("attention_dropout", 0.2),
            cfg.get("relu_dropout", 0.2),
        )

        self.self_attn_layers = bi_directional_self_attention_layers(*params)
        self.output_layer = SoftmaxOutputLayer(
            hidden_size=cfg.get("hidden_size", 128),
            output_size=cfg.get("num_chords", 170),
