...
- **検知解像度（ビート分割単位）について**
  - **`0.25` (16分音符単位 / デフォルト・推奨)**: 1拍を4分割してコードを検出します。小節末尾のわずかなフライング食い込み（0.25拍）をノイズとして除去しつつ、8分音符（0.5拍）以上の本来のアウフタクトを正確に保持します。
  - **`0.5` (8分音符単位 / 標準)**: 1拍を2分割してコードを検出します。従来通りの大まかな8分音符単位でコードを推定・平滑化します。
